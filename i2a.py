...
class I2A (object):
    ...
    def replay_pixel(self):
        ...
        for i in range(batch_size):
            ...
            td_err = target1 - self.critic.predict([np.array([encoded_state]), np.array([X_c2[i]])], batch_size = 1)[0]
            if td_err>0.:
    ...
    def replay_latent(self):
        ...
        X_c1, X_c2, X_a, = np.zeros((batch_size,latent_dim)), np.zeros((batch_size,1)), []
        ...
        for i in range(batch_size):
            ...
            td_err = target - self.critic.predict([np.array([latent_state]), np.array([X_c2[i]])], batch_size = 1)[0]
            if td_err>0.:
                X_a.append(latent_state); Y_a.append(action)
    ...
    def learn (self, nb_episodes = 10000, steps_per_episode = 50, sim = 1, nb_simulation = 1):
        ...
        for i_episode in range(nb_episodes):
            ...
            for t in range(steps_per_episode):
                ...
                self.remember_pixel(observation, clipped_act, r_ext, r_total, obs_new, done)                          
